...
*/
bool Periphery::InitI2C2()
{
	bool function_failed = false;
	
	RCC->APBENR1 |= RCC_APBENR1_I2C2EN;												// Enable I2C tick
	...
	I2C2->CR1 &= ~I2C_CR1_ANFOFF;															// Disable analog filter
	I2C2->CR1 &= ~I2C_CR1_NOSTRETCH;													// Disable clock stretching
	
/*
	I2C2->TIMINGR |= (I2C_TIMINGR_SCLL & 0x13);
	I2C2->TIMINGR |= (I2C_TIMINGR_SCLH & (0x0F << I2C_TIMINGR_SCLH_Pos));
	I2C2->TIMINGR |= (I2C_TIMINGR_SDADEL & (0x00 << I2C_TIMINGR_SDADEL_Pos));
	I2C2->TIMINGR |= (I2C_TIMINGR_SCLDEL & (0x01 << I2C_TIMINGR_SCLDEL_Pos));
	I2C2->TIMINGR |= (I2C_TIMINGR_PRESC & (0x0B << I2C_TIMINGR_PRESC_Pos));
*/

	// 400 kHz @ 8MHz tick
	I2C2->TIMINGR|= (I2C_TIMINGR_SCLL & 0x08);
	I2C2->TIMINGR|= (I2C_TIMINGR_SCLH & (0x03 << I2C_TIMINGR_SCLH_Pos));
	I2C2->TIMINGR |= (I2C_TIMINGR_SDADEL & (0x00 << I2C_TIMINGR_SDADEL_Pos));
	I2C2->TIMINGR |= (I2C_TIMINGR_SCLDEL & (0x01 << I2C_TIMINGR_SCLDEL_Pos));
	I2C2->TIMINGR |= (I2C_TIMINGR_PRESC & (0x00 << I2C_TIMINGR_PRESC_Pos));

	
	
	// I2C2->CR1 |= I2C_CR1_RXIE; 														// Enable reciver interupt
	// I2C2->CR1 |= I2C_CR1_TCIE;															// Enable tranciver interrupt
	// I2C2->CR2 |= I2C_CR2_AUTOEND;													// Master send STOP condition automatically, when NBYTES[7:0] were transmitted
...
{	
	...
	if (Direction) 
	{ 
		I2C2->CR2 |= I2C_CR2_RD_WRN;													// Receiver
	}	
	else 
	{
		I2C2->CR2 &= ~I2C_CR2_RD_WRN;													// Transmitter
	}

	I2C2->CR2 &= ~I2C_CR2_NBYTES;														// Clear data size
	I2C2->CR2 |= Size << I2C_OFFSET_CR2_NBYTES;							// Set data size
	
	I2C2->CR2 &= ~I2C_CR2_SADD;															// Clear slave address
...
*/
bool Periphery::WriteI2C2_DS3231(const uint8_t device_address, const uint8_t data_command, uint8_t *data, const uint8_t data_amount)
{
	if(data_amount == 0xFF) {
		return 1;
	}
	...
	*(uint8_t *)&(I2C2->TXDR) = data_command;
	...
	if(data_amount) {
		...
		while (!(I2C2->ISR & I2C_ISR_TC) && !(I2C2->ISR & I2C_ISR_NACKF))
		{
			...
			if (I2C2->ISR & I2C_ISR_TXIS) {
				I2C2->TXDR =*(data + data_count++);										// send data
...
*/
bool Periphery::ReadI2C2_DS3231(const uint8_t device_address, const uint8_t data_command, uint8_t *data, const uint8_t data_amount)
{
	bool function_failed = false;
	...
	*(uint8_t *)&(I2C2->TXDR) = data_command;
	...
	uint16_t cnt4 = 6000;
	while ( (((I2C2->ISR & I2C_ISR_TC) == 0) && ((I2C2->ISR & I2C_ISR_NACKF) == 0)) || (I2C2->ISR & I2C_ISR_RXNE) != 0)
	{
		if (I2C2->ISR & I2C_ISR_RXNE) 
		{	
			*(data + data_count++) = I2C2->RXDR;
			cnt4 = 6000;
		}
		if(cnt4 == 0) 
		{ 
